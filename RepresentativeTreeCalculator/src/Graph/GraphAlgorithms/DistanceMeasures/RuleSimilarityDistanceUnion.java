...
public class RuleSimilarityDistanceUnion extends TreeDistanceMeasure<DecisionTreeNode, DecisionTreeEdge> {
    @Expose
    String name = "RuleSimilarityDistanceUnion";
    private final List<MinMaxValues> baseMinMaxValues = new ArrayList<>();
    private final int SCALE_FACTOR = 1000;

    public RuleSimilarityDistanceUnion(String inputFileLocation) throws IOException {
    ...
    private Double embeddingFunction(RuleIntervals ruleIntervals1, RuleIntervals ruleIntervals2) {
        ...
        for (int i = 0; i < intervalSizes; i++) {
            ...
            double union = Math.max(Math.max(values1.maxValue, values2.maxValue) - Math.min(values1.minValue, values2.minValue), 0.0);
            double distance = 1 - overlay / union;
