...
public class RuleSimilarityDistanceSum extends TreeDistanceMeasure<DecisionTreeNode, DecisionTreeEdge> {
    @Expose
    String name = "RuleSimilarityDistanceSum";
    private final List<MinMaxValues> baseMinMaxValues = new ArrayList<>();
    private final int SCALE_FACTOR = 1000;

    public RuleSimilarityDistanceSum(String inputFileLocation) throws IOException {
    ...
    private Double embeddingFunction(RuleIntervals ruleIntervals1, RuleIntervals ruleIntervals2) {
        ...
        for (int i = 0; i < intervalSizes; i++) {
            ...
            double sum = (values1.maxValue - values1.minValue) + (values2.maxValue - values2.minValue);
            double distance = 1 - 2 * overlay / sum;
